...
from alignments.DFLJPG import DFLJPG
from alignments.DFLPNG import DFLPNG
...
def align_face_dfl(filepath, predictor):
    """
    :param filepath: str
    :return: PIL Image
    """
    ...
    # aligned_folder = '/home/ubuntu/data/psp/frame/10_aligned_wf/'
    # aligned_filepath = aligned_folder + '.'.join(os.path.basename(filepath).split('.')[:-1]) + '.jpg'
    # face_data = DFLJPG.load(aligned_filepath)
    # landmarks =face_data.get_landmarks()
    # lm = landmarks
    # print(landmarks)

    aligned_folder = '/home/ubuntu/data/psp/frame/Trump_aligned_wf/'
    aligned_filepath = aligned_folder + '.'.join(os.path.basename(filepath).split('.')[:-1]) + '_0.jpg'
    face_data = DFLJPG.load(aligned_filepath)
    landmarks =face_data.get_landmarks()
    mat = face_data.get_image_to_face_mat()
